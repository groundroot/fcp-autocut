...
- **타임라인의 모든 오디오를 인식** — 메인 스토리라인은 물론, **커넥트 클립·연결 스토리라인**('스토리라인 만들기')에 붙인 오디오도 연결 방식·중첩 깊이와 무관하게 찾아 자막을 만듭니다
- **의미 단위 자막 분리 엔진** — 문장 전체를 보고 **가장 자연스러운 분리 조합**을 계산(동적계획법). 보조용언("말하고 싶은")·의존명사("수 있도록")·숫자+단위("열한 명")·접속부사를 쪼개지 않습니다. 문장별 예외가 아닌 **단일 일반 로직**
- **자막 스타일 실시간 프리뷰** — 사이드바 "자막 스타일"에서 글자 크기(10단위)·자막 길이(10구간)·**2줄 자막**·위치(직접 드래그)를 실시간으로 확인하며 조절. 입력 프로젝트의 **화면비(16:9/9:16)를 자동 감지**해 그 비율로 표시하고, 프리뷰의 글자 크기·위치가 **파이널컷 실제 렌더와 1:1로 일치**합니다. 9:16에는 유튜브·인스타그램·틱톡 **세이프존 가이드** 표시 가능
- **처음부터 원하는 타이틀 스타일로 생성** — Vrew·Whisper 계열 도구로 만든 자막은 FCP에서 다른 타이틀로 교체하면 길이가 리셋되며 타임라인이 어긋납니다(템플릿 내부 시작점 불일치가 원인). FCP AutoCut은 **타이틀 스타일을 처음부터 선택해 템플릿 네이티브 값으로 생성**하므로 교체 자체가 필요 없고, 어긋남이 생기지 않습니다. **FCP 12.3의 자막 전용 Subtitle 타이틀 + Subtitles 롤**도 지원(전체 선택·일괄 편집 대상)
- **혼합 프레임레이트 안전** — 30p 프로젝트 + 29.97fps 소스처럼 시퀀스와 미디어의 레이트가 달라도 자막이 **미디어 프레임 격자**에 정확히 정렬됩니다 (edit frame boundary 경고 없음)
- **결과물** — ① 원본 타임라인 + 자막 ② 무음 제거로 압축된 타임라인 + 자막 *(무음컷을 켜고 잘라낼 무음이 있을 때 생성)*
- **작업 히스토리** — 처리한 프로젝트 기록(최대 50건)에서 결과 폴더를 바로 열기
...
| **고유명사 교정** | **단어별로 등록**한 용어(등록 단어 + 잘못 들리는 발음)로 자막을 통일 (예: "제리안이"→"정예린이", 조사 보존). 적용 전 **검토 창**에서 확인·수정. 사전은 **JSON으로 내보내기/가져오기**(백업·복원) 가능 — 토글 옆 톱니 버튼으로 바로 편집 | 즉시·무료, 모델 불필요 |

### 조절 가능한 옵션

자막 자동화 · 언어 선택 · 자막 형식(타이틀/캡션/둘 다) · **자막 스타일 프리뷰**(글자 크기·자막 길이·2줄·위치 드래그·세이프존 가이드) · **타이틀 스타일**(FCP 12.3 Subtitle/Custom/Basic) · Subtitles 롤(12.3) · 빈틈없이 채우기 · 무음컷 · 컷 강도 · 군소리 컷("어/음/아") · 화자 분리 · 고유명사 교정

## 성능

- **자막 인식·정밀 타이밍**: 영상 길이에 비례 (M1 기준 대략 실시간의 몇 배 빠르게 처리)
- **자막 분리·고유명사 교정**: 규칙 기반이라 **즉시** (모델 불필요)
...
- 네트워크는 **AI 모델 최초 다운로드**, **업데이트 확인**, 그리고 사용자가 직접 실행할 때의 **피드백 전송·구매(App Store)** 에만 사용됩니다 — 영상·음성 처리에는 쓰이지 않습니다
