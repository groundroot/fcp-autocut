...
| **용어 사전 치환** | **단어별로 등록**한 용어(등록 단어 + 잘못 들리는 발음)로 자막을 통일 (예: "제리안이"→"정예린이", 조사 보존). 적용 전 **검토 창**에서 확인·수정. 사전은 **JSON으로 내보내기/가져오기**(백업·복원) 가능 — 토글 옆 톱니 버튼으로 바로 편집 | 즉시·무료, 모델 불필요 |

### 조절 가능한 옵션

자막 자동화 · 언어 선택 · 자막 형식(타이틀/캡션/둘 다) · **자막 스타일 프리뷰**(글자 크기·자막 길이·2줄·위치 드래그·세이프존 가이드) · **타이틀 스타일**(FCP 12.3 Subtitle/Custom/Basic) · Subtitles 롤(12.3) · 빈틈없이 채우기 · 무음컷 · 컷 강도 · 군소리 컷("어/음/아") · 화자 분리 · 용어 사전 치환
