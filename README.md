...
- **처음부터 원하는 타이틀 스타일로 생성** — Vrew·Whisper 계열 도구로 만든 자막은 FCP에서 다른 타이틀로 교체하면 길이가 리셋되며 타임라인이 어긋납니다(템플릿 내부 시작점 불일치가 원인). FCP AutoCut은 **타이틀 스타일을 처음부터 선택해 템플릿 네이티브 값으로 생성**하므로 교체 자체가 필요 없고, 어긋남이 생기지 않습니다
...
자막 자동화 · 언어 선택 · 자막 형식(타이틀/캡션/둘 다) · 글자 크기 · 자막 길이 · **타이틀 스타일** ·
빈틈없이 채우기 · 무음컷 · 컷 강도 · 군소리 컷("어/음/아") · 다시 말하기 컷 · 화자 분리 · 용어 사전 치환
